    ...
    <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-top-transition">
      <template v-slot:activator="{ on, attrs }">
        <button v-bind="attrs" v-on="on">
          <div class="tw-flex tw-flex-col tw-items-center tw-p-2 tw-space-y-2">
            <div class="tw-bg-[#FECDA4] tw-w-10 tw-h-10 tw-flex tw-justify-center tw-rounded-full">
              <v-icon color="#6a8684">mdi-wallet</v-icon>
            </div>
            <span class="tw-font-extrabold tw-text-sm tw-text-[#6a8684]">Novo</span>
          </div>
        </button>
      </template>
